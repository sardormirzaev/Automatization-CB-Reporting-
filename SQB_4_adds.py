...
#%%
    ...
    df2['ДатаОбразПрос']=df2['ДатаОбразПрос'].replace('nan',0)
    df2['ДатаОбразПрос']=df2['ДатаОбразПрос'].\
    apply(lambda x : x if x !='01.01.1970' else 0)
    df2['ДатаОбразПросПроц']=df2['ДатаОбразПросПроц'].replace('nan',0)
    df2['ДатаОбразПросПроц']=df2['ДатаОбразПросПроц'].\
    apply(lambda x : x if x !='01.01.1970' else 0)
    df2['loop']=[pd.to_datetime(i,format= '%d.%m.%Y',errors='ignore') \
       for i in df2['ДатаОбразПрос']]
    lov=[]
    for i in df2['loop']:
        if i!=0:
            lov.append(i-obs_date)
        else:
            lov.append(i)
    
    lov=[str(abs(i)) for i in lov]
    lov=[i.replace('days 00:00:00','') for i in lov]  
    
    df2['loop2']=[pd.to_datetime(i,format= '%d.%m.%Y',errors='ignore') \
           for i in df2['ДатаОбразПросПроц']]
    lov2=[]
    for i in df2['loop2']:
        if i!=0:
            lov2.append(i-obs_date)
        else:
            lov2.append(i)
    
    lov2=[str(abs(i)) for i in lov2]
    lov2=[i.replace('days 00:00:00','') for i in lov2]

    df2['Прос куни'] =[int(i) for i in lov]
    # Дни просрочки процентной задолженности
    #pros_procen = pd.to_datetime(df2['ДатаОбразПросПроц'])
    df2['ФОИЗ КУНИ']=[int(i) for i in lov2]
  ...
  #%%
    ...
    df2['тип клиента']=df2['тип клиента'].apply(lambda x : x if type(x)==str\
       else 'Частные товарищества и корпорации')
    
    df2['средневзвеш']=df2['ВсегоЗадолженность']*\
    (df2['% кредита'].apply(lambda x: x/100))

    nf=df2[df2['тип клиента'] == \
      'Негосударственные некоммерческие организации']
    nf['СекторКлиентаДляОтчетов']=str('7-Жилищно-коммунальные услуги') 
    
    df2['СекторКлиентаДляОтчетов1']=nf['СекторКлиентаДляОтчетов'].\
    combine_first(df2['СекторКлиентаДляОтчетов'])
    df2['sab']=np.where((df2['СекторКлиентаДляОтчетов1']\
       =='7-Жилищно-коммунальные услуги')\
        &(df2['тип клиента']!='Негосударственные некоммерческие организации'),\
        '8-Другие отрасли',df2['СекторКлиентаДляОтчетов1'])
        
    df2['СекторКлиентаДляОтчетов2']=df2['sab']     
    df2['otr']=df2['СекторКлиентаДляОтчетов2'].str[0].astype(int)
    df2['Отрасль_eng']=df2['otr'].apply(lambda x: func.cat_industr(x))
    df2['Отрасль'] = df2['СекторКлиентаДляОтчетов2'].str[0]
    df2['СекторКлиентаДляОтчетов']=df2['СекторКлиентаДляОтчетов2']
    df2['ОКЭД'] = df2['ОКЭД клиента'].str[:5]
    
    #%%
    df3=df2[['средневзвеш','СекторКлиентаДляОтчетов','MFO_schet',\
             'тип клиента','КодВал2','Прос куни',\
             'ФОИЗ КУНИ','Резерв%' ,'КодОбес','ОКЭД','Отрасль','ракам клас',\
             'Классификация','Баланс х/р','Уникал',
    ...
    daf = daf.loc[:,~daf.columns.duplicated(keep='first')]
...
def datetrim(df):
    ...
    for c in dfs[['СуммаДог(ном)','СуммаДог(сум.экв)',\
                  ...
                  'Остаток пени','ОценкаОбеспечения',\
                  'ОстатокВнебПроцентов']]:
...
#        #dfs[c] = dfs[c].apply(lambda x : func.convert_float_int(x))
    ...
    del dfs['СекторКлиентаДляОтчетов']   
    return dfs.iloc[:,:-5]

def add_top_rows(df,dfs):
    import SQB_4_functions as func
    dfs.NN[0:4]=df.iloc[0:4,0]
    dfs.средневзвеш.loc[2]=func.int_float_none(dfs['средневзвеш'][5:].sum())
